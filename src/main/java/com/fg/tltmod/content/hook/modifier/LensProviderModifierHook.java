...
public interface LensProviderModifierHook {
    default List<ItemStack> getLens(IToolStackView tool, ModifierEntry modifier, List<ModifierEntry> modifierList, ManaBurst burst){
        return List.of();
    }
    static List<ItemStack> gatherLens(IToolStackView tool,ManaBurst burst){
        ...
        tool.getModifierList().forEach(entry -> {
            ...
            else list.addAll(entry.getHook(TltCoreModifierHook.LENS_PROVIDER).getLens(tool,entry,tool.getModifierList(),burst));
        });
        return list;
    }
    record AllMerger(Collection<LensProviderModifierHook> modules) implements LensProviderModifierHook {
        ...
        public List<ItemStack> getLens(IToolStackView tool, ModifierEntry modifier, List<ModifierEntry> modifierList, ManaBurst burst) {
            ...
            for (var entry:this.modules){
                list.addAll(entry.getLens(tool,modifier,modifierList,burst));
            }
            return list;
        }
    }
