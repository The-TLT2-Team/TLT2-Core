...
public class RenderWaveSlash extends EntityRenderer<WaveSlashEntity> {
    ...
    protected RenderWaveSlash(EntityRendererProvider.Context pContext) {
        super(pContext);
    }
    ...
    public void render(WaveSlashEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        float radius = 1.5f*percent;
        double deg = 30;
        float d = Math.min(tick,4);
        for (int i =0;i<16;i++){
            deg+=15;
            float z0 = (float) Math.cos(deg)*radius;
            float z1 = (float) Math.cos(deg+15)*radius;
            float x0 = (float) Math.sin(deg)*radius;
            float x1 = (float) Math.sin(deg+15)*radius;
            float a0 = (float) (1-Math.pow((90-deg)/90,2))*percent;
            float a1 = (float) (1-Math.pow((75-deg)/90,2))*percent;
            renderRhomboid(0.4f,x0,x1,z0,z1,0.05f,255,255,255,
                    getR(colTick),getG(colTick),getB(colTick),Math.min(a0*2,1),0,0,Math.min(a1*2,1),
                    normalMatrix,poseMatrix,consumer);
            renderRhomboid(0.4f,x0,x1,z0,z1,-0.05f,255,255,255,
                    getR(colTick),getG(colTick),getB(colTick),Math.min(a0*2,1),a0,a1,Math.min(a1*2,1),
                    normalMatrix,poseMatrix,consumer);
            renderRhomboid(0.4f,x0,x1,z0-0.05f,z1-0.05f,-d,getR(colTick),getG(colTick),getB(colTick),
    ...
    public static void renderRhomboid(float y,float x0,float x1,float z0,float z1,float d,int r,int g,int b,int r1,int g1,int b1,float a00,float a01,float a11,float a10,
        ...
        consumer.vertex(poseMatrix,x0,y,z0).uv(0,0).color(r,g,b,a00)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT)
                .normal(normalMatrix, 0, -1, 0).endVertex();
        consumer.vertex(poseMatrix,x0,y,z0+d).uv(0,1).color(r1,g1,b1,a01)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT)
                .normal(normalMatrix, 0, -1, 0).endVertex();
        consumer.vertex(poseMatrix,x1,y,z1+d).uv(1,1).color(r1,g1,b1,a11)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT)
                .normal(normalMatrix, 0, -1, 0).endVertex();
        consumer.vertex(poseMatrix,x1,y,z1).uv(1,0).color(r,g,b,a10)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT)
                .normal(normalMatrix, 0, -1, 0).endVertex();
    }
